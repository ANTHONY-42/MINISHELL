...
int main() {
    ...
    sigemptyset(&mask);  // Initialise l'ensemble à vide

    // Remplir la structure sigaction
    memset(&sa, 0, sizeof(sa));  // Initialiser la structure à zéro
    sa.sa_handler = handle_signal;  // Définir le gestionnaire de signal
    sa.sa_mask = mask;  // Le masque de signaux à bloquer pendant l'exécution
